"""
LangGraph Nodes for IRCoT Orchestration.

This module defines the individual nodes (functions) that process state
in the LangGraph workflow. Each node represents a step in the IRCoT algorithm.

Node Functions:
- plan_node: Analyze query, determine complexity, extract filters
- retrieve_node: Retrieve documents from RAG/Knowledge Graph
- reason_node: Generate chain-of-thought reasoning step
- answer_node: Generate final answer from accumulated context
- check_node: Decide whether to continue IRCoT loop

Reference: LangGraph node conventions
"""
...
class LangGraphNodes:
    """
    Collection of node functions for the IRCoT LangGraph workflow.
    
    Each method is a node that takes state and returns updated state.
    Nodes are designed to be composable and reusable.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            smart_planner: SmartPlanner agent for query analysis
            answer_agent: Agent for generating final answers
            graph_reasoning_agent: Agent for Knowledge Graph reasoning
            conversation_manager: Manager for conversation history
            context_service: Service for query contextualization
            ircot_config: Configuration for IRCoT behavior
        """
...
Sử dụng tiếng Việt cho nội dung reasoning và thông tin."""
    ...
    async def retrieve_node(self, state: IRCoTState) -> Dict[str, Any]:
        """
        Retrieval node: Retrieve documents from RAG and/or Knowledge Graph.
        
        This node:
        1. Gets documents from vector search (RAG)
        2. Optionally gets context from Knowledge Graph
        3. Merges and deduplicates results
        4. Accumulates documents across iterations
