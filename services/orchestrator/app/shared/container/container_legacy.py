...
from ...chat.agents.optimized_orchestrator import OptimizedMultiAgentOrchestrator
...
from ...shared.config.config_manager import ConfigurationManager, get_config_manager
from .agent_factory import AgentFactory, ConfigurableAgentFactory, get_agent_factory
from ...shared.config.ircot_config import IRCoTConfig, IRCoTMode

# Import Symbolic Reasoning components (lazy import to avoid circular dependencies)
...
# Ensure services/ is on sys.path so we can import rag_services as a proper package
...
class ServiceContainer:
    def __init__(self, config_path: Optional[str] = None):
        ...
        self._symbolic_graph_extension = None  # Symbolic reasoning extension
        self._symbolic_reasoning_engine = None  # Main symbolic reasoning engine
    ...
    def get_graph_adapter(self):
        if self._graph_adapter is None:
            ...
            try:
                # Import Neo4jGraphAdapter via proper package path
                # (services/ is on sys.path, so rag_services is a regular package)
                from rag_services.app.knowledge_graph.adapters.neo4j_adapter import Neo4jGraphAdapter
    ...
    def get_symbolic_graph_extension(self):
        if self._symbolic_graph_extension is None:
            ...
            try:
                SymbolicReasoningEngine, ReasoningMode, SymbolicGraphExtension = _get_symbolic_reasoning_engine()
    ...
    def get_symbolic_reasoning_engine(self):
        if self._symbolic_reasoning_engine is None:
            # Check if symbolic reasoning is enabled
            ...
            # Get symbolic graph extension (or raw adapter)
            ...
            try:
                ...
                # Get reasoning mode from environment
                mode_str = os.getenv("SYMBOLIC_REASONING_MODE", "hybrid").lower()
                if mode_str == "rule_based":
                ...
                # Create engine with optional LLM for hybrid mode
...
def get_symbolic_reasoning_engine():
    """Get global symbolic reasoning engine instance."""
    container = get_container()
    return container.get_symbolic_reasoning_engine()


def get_symbolic_graph_extension():
    """Get global symbolic graph extension instance."""
    container = get_container()
    return container.get_symbolic_graph_extension()
