...
# It supports both traditional vector search and hybrid search (BM25 + Vector + Cross-Encoder).
# The engine can perform vector-only search, BM25-only search, or hybrid fusion search.
#
# Key Responsibilities:
# - Loading the LlamaIndex VectorStoreIndex from disk.
# - Providing hybrid search capabilities with BM25 + Vector fusion.
...
_engine = None
def get_query_engine() -> HybridEngine:
    """
    Factory function to get the singleton instance of the HybridEngine.

    Returns:
        HybridEngine: The singleton hybrid query engine instance.
    """
    global _engine
    if _engine is None:
